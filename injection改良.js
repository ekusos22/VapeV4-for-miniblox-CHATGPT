		...
		(function() {
            ...
            new Module("ESP", function(callback) {
			    if (callback) {
			        ...
			        let killauraRange = 1000; // Set the unlimited range (or any large value)
			        ...
			        tickLoop["ESP"] = function() {
			            ...
			            for (const entity of entities.values()) {
			                ...
			                // Check if the entity is within the Killaura range (unlimited range)
			                let isInKillauraRange = player$1.getDistanceSqToEntity(entity) < (killauraRange * killauraRange);

			                if (isInKillauraRange) {
			                    // Highlight the entity in red
			                    for (const mesh in entity.mesh.meshes) {
			                        let material = entity.mesh.meshes[mesh].material;

			                        // Set color to red
			                        material.color = new THREE.Color(1, 0, 0); // Red color
			                        material.transparent = true;
			                        material.opacity = 0.5;  // Adjust transparency
			                        material.blending = THREE.NormalBlending;  // Set blending mode
			                        material.depthTest = false;  // Disable depth testing

			                        material.needsUpdate = true;  // Ensure material updates
			                        entity.mesh.meshes[mesh].renderOrder = 6;  // Render on top
			                    }
			        ...
			        for (const entity of game$1.world.entitiesDump.values()) {
			            for (const mesh in entity.mesh.meshes) {
			                ...
			                material.color = new THREE.Color(1, 1, 1);  // Set back to default color
			                material.transparent = false;
			                material.opacity = 1;  // Reset transparency
			                material.depthTest = true;  // Enable depth testing again
			                material.needsUpdate = true;
			                entity.mesh.meshes[mesh].renderOrder = 0;  // Reset render order
			            }
			        }
			    }
			});
